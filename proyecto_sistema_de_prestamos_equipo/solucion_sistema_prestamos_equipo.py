...
# Estructura principal: Diccionario de equipos
# El estado 'True' significa disponible
inventario = {
...
def registrar_prestamo():
    ...
    # Validamos si existe y si no está ocupado
    if nombre_equipo in inventario:
        if inventario[nombre_equipo]["disponible"]:
            usuario = input("Nombre del usuario: ")
            fecha = datetime.date.today().strftime("%d/%m/%Y")
            
            # Guardamos el préstamo como una tupla (inmutable)
...
def agregar_equipo():
    ...
    if nuevo_nombre not in inventario:
        # Agregamos al diccionario con valores iniciales
